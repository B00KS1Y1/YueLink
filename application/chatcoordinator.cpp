...
, m_conversations(std::make_unique<ConversationStore>(std::move(repository)))
{
    ...
    m_transfers = std::make_unique<TransferCoordinator>(m_transport.get(), m_conversations.get(), &m_identity);
    connectServices();
}
...
bool ChatCoordinator::conversation(const QString &conversationId, Domain::Conversation *result) const
{
...
QList<Domain::Message> ChatCoordinator::messages(const QString &conversationId, int limit)
{
    return m_conversations->messages(conversationId, limit);
}
void ChatCoordinator::deliveryCounts(const QString &messageId, int *deliveredCount, int *totalCount) const
{
...
Domain::OperationResult ChatCoordinator::start()
{
    ...
    if (!m_identityReady || m_identity.deviceId.isEmpty() || m_identity.displayName.isEmpty())
    {
        const QString error = m_lastError.isEmpty() ? tr("本机身份信息无效。") : m_lastError;
        setLastError(error);
    ...
    if (!m_transport->start(m_identity))
    {
        ...
        return Domain::OperationResult::failure(QStringLiteral("transport.start"), m_lastError);
    }
    if (!m_discovery->start(m_identity, m_transport->listeningPort()))
    {
        setLastError(m_discovery->lastError());
        m_transport->stop();
        return Domain::OperationResult::failure(QStringLiteral("discovery.start"), m_lastError);
    }
    m_running = true;
    setLastError({});
    emit runningChanged();
...
Domain::OperationResult ChatCoordinator::refreshPeerDiscovery()
{
    if (!m_running)
    {
        return Domain::OperationResult::failure(QStringLiteral("discovery.not_running"), tr("局域网服务未启动。"));
    }
    m_discovery->probe();
    return Domain::OperationResult::success();
}

Domain::OperationResult ChatCoordinator::markConversationRead(const QString &conversationId)
{
...
Domain::OperationResult ChatCoordinator::createGroup(const QString &name, const QStringList &memberIds)
{
    ...
    {
        return Domain::OperationResult::failure(QStringLiteral("group.invalid_name"), tr("群名称需要包含 1 到 64 个字符。"));
    }
    ...
    {
        return Domain::OperationResult::failure(QStringLiteral("group.invalid_members"), tr("请选择 2 到 31 位联系人。"));
    }

    Domain::Group group;
    group.groupId = QStringLiteral("group:%1").arg(QUuid::createUuid().toString(QUuid::WithoutBraces));
    group.name = normalizedName;
    group.ownerId = m_identity.deviceId;
    group.createdAt = QDateTime::currentDateTimeUtc();
    group.members.append({m_identity.deviceId, m_identity.displayName, Domain::GroupRole::Owner});
    for (const QString &memberId : uniqueIds)
    {
        Domain::Peer peerRecord;
        if (!m_conversations->peer(memberId, &peerRecord))
        {
            return Domain::OperationResult::failure(QStringLiteral("group.unknown_member"), tr("部分联系人已经不可用。"));
        }
        group.members.append({memberId, peerRecord.endpoint.displayName, Domain::GroupRole::Member});
    }
    if (!m_conversations->upsertGroup(group))
    {
        return Domain::OperationResult::failure(QStringLiteral("group.save"), tr("无法保存群聊。"));
    }
    ...
    {
        Domain::Peer peerRecord;
        if (m_running && m_conversations->peer(memberId, &peerRecord) && peerRecord.online)
        {
...
Domain::OperationResult ChatCoordinator::updateLocalProfile(const QString &displayName, const QString &avatarPath, const QString &avatarColor)
{
    const QString normalizedName = displayName.trimmed();
    if (normalizedName.isEmpty() || normalizedName.size() > 64)
    {
        return Domain::OperationResult::failure(QStringLiteral("profile.invalid_name"), tr("昵称需要包含 1 到 64 个字符。"));
    }
    ...
    if (!normalizedAvatarPath.isEmpty())
    {
        ...
        {
            return Domain::OperationResult::failure(QStringLiteral("profile.invalid_avatar"), tr("头像图片无效或不可读取。"));
        }
    ...
    {
        return Domain::OperationResult::failure(QStringLiteral("profile.invalid_color"), tr("头像颜色无效。"));
    }
    ...
    if (!result)
    {
        return Domain::OperationResult::failure(QStringLiteral("identity.save"), result.errorMessage);
    }
    ...
    if (previousName != normalizedName)
    {
        for (const Domain::Conversation &conversation : m_conversations->conversations())
        {
            ...
            Domain::Group group;
            if (!m_conversations->group(conversation.conversationId, &group) || group.ownerId != m_identity.deviceId)
            {
                continue;
            }
            ...
            {
                Domain::Peer peerRecord;
                if (member.peerId != m_identity.deviceId && m_running && m_conversations->peer(member.peerId, &peerRecord) && peerRecord.online)
                {
                    m_transport->sendGroupSnapshot(peerRecord.endpoint, snapshot);
                }
            }
...
Domain::OperationResult ChatCoordinator::sendText(const QString &conversationId, const QString &text)
{
...
Domain::OperationResult ChatCoordinator::sendImage(const QString &conversationId, const QString &filePath, const QString &caption)
{
    ...
    {
        return Domain::OperationResult::failure(QStringLiteral("image.caption_too_long"), tr("图片说明不能超过 2000 个字符。"));
    }
    QImageReader reader(filePath);
    const QSize dimensions = reader.size();
    if (!dimensions.isValid())
    {
        return Domain::OperationResult::failure(QStringLiteral("image.invalid"), tr("无法读取图片信息。"));
    }
    ...
    return sendPayload(conversationId, Domain::ImagePayload{std::move(descriptor), dimensions, normalizedCaption}, QFileInfo(filePath).absoluteFilePath());
}

int ChatCoordinator::sendImages(const QString &conversationId, const QStringList &filePaths)
{
...
Domain::OperationResult ChatCoordinator::sendEmoji(const QString &conversationId, const QString &packageId, const QString &emojiId, const QString &fallbackText)
{
    const Domain::EmojiPayload payload{packageId.trimmed(), emojiId.trimmed(), fallbackText.trimmed()};
    if (payload.packageId.isEmpty() || payload.packageId.size() > 128 || payload.emojiId.isEmpty() || payload.emojiId.size() > 128 ||
        payload.fallbackText.isEmpty() || payload.fallbackText.size() > 64)
    {
        return Domain::OperationResult::failure(QStringLiteral("emoji.invalid"), tr("表情信息无效。"));
    }
    return sendPayload(conversationId, payload);
}

Domain::OperationResult ChatCoordinator::sendPayload(const QString &conversationId, Domain::MessagePayload payload, const QString &localPath)
{
    ...
    const bool attachment = std::holds_alternative<Domain::ImagePayload>(payload) || std::holds_alternative<Domain::FilePayload>(payload);
    if (attachment && conversation.kind != Domain::ConversationKind::Direct)
    {
        return Domain::OperationResult::failure(QStringLiteral("attachment.group_unsupported"), tr("群聊暂不支持附件。"));
    }
    ...
    if (conversation.kind == Domain::ConversationKind::Group)
    {
        ...
        {
            return Domain::OperationResult::failure(QStringLiteral("group.unknown"), tr("群聊不存在。"));
        }
        ...
        if (!localMember)
        {
            return Domain::OperationResult::failure(QStringLiteral("group.not_member"), tr("你已不在该群聊中。"));
        }
    }
    ...
    message.deliveryState = attachment ? Domain::DeliveryState::Transferring : Domain::DeliveryState::Sending;
...
Domain::OperationResult ChatCoordinator::sendFile(const QString &conversationId, const QString &filePath)
{
    ...
    return sendPayload(conversationId, Domain::FilePayload{std::move(descriptor)}, QFileInfo(filePath).absoluteFilePath());
}
...
bool ChatCoordinator::attachmentDescriptor(const QString &filePath, Domain::AttachmentDescriptor *descriptor, QString *errorMessage) const
{
    ...
    if (value.fileName.size() > 255 || value.fileSize < 0 || value.fileSize > MaximumAttachmentSize)
    {
        if (errorMessage)
        {
...
Domain::OperationResult ChatCoordinator::cancelFileTransfer(const QString &conversationId, const QString &transferId)
{
    ...
    if (peerId.isEmpty())
    {
        return Domain::OperationResult::failure(QStringLiteral("file.group_unsupported"), tr("群聊暂不支持文件传输。"));
    }
    return m_transfers->cancel(peerId, transferId);
}

Domain::OperationResult ChatCoordinator::acceptFileTransfer(const QString &conversationId, const QString &transferId)
{
    const QString peerId = Domain::peerIdFromDirectConversation(conversationId);
    if (peerId.isEmpty())
    {
...
void ChatCoordinator::connectServices()
{
    connect(m_discovery.get(), &IPeerDiscovery::peerFound, this, [this](const Network::PeerEndpoint &endpoint) {
        ...
        const bool shouldSynchronizeGroups = !known || !previous.online || previous.endpoint.displayName != endpoint.displayName ||
                                             previous.endpoint.address != endpoint.address || previous.endpoint.tcpPort != endpoint.tcpPort;
        m_conversations->observePeer(endpoint);
        Domain::Peer peerRecord;
        if (shouldSynchronizeGroups && m_conversations->peer(endpoint.peerId, &peerRecord))
        {
            synchronizeOwnedGroupsWithPeer(peerRecord);
        }
        dispatchPendingToPeer(endpoint.peerId);
    });
    connect(m_discovery.get(), &IPeerDiscovery::peerLost, m_conversations.get(), &ConversationStore::markPeerOffline);
    connect(m_discovery.get(), &IPeerDiscovery::errorOccurred, this, &ChatCoordinator::setLastError);
    connect(m_transport.get(), &IChatTransport::peerObserved, this, [this](const Network::PeerEndpoint &endpoint) {
    ...
    connect(m_conversations.get(), &ConversationStore::messageAdded, this, &ChatCoordinator::messageAdded);
    connect(m_conversations.get(), &ConversationStore::messageStateChanged, this, &ChatCoordinator::messageStateChanged);
    connect(m_conversations.get(), &ConversationStore::deliveryChanged, this, &ChatCoordinator::deliveryChanged);
    connect(m_conversations.get(), &ConversationStore::fileTransferChanged, this, &ChatCoordinator::fileTransferChanged);
    connect(m_conversations.get(), &ConversationStore::operationFailed, this, &ChatCoordinator::operationFailed);

    connect(m_transfers.get(), &TransferCoordinator::fileReceived, this, [this](const QString &peerId, const QString &path) {
        emit fileReceived(Domain::directConversationId(peerId), path);
    });
    connect(m_transfers.get(), &TransferCoordinator::fileTransferFailed, this, [this](const QString &peerId, const QString &reason, bool incoming) {
        emit fileTransferFailed(Domain::directConversationId(peerId), reason, incoming);
    });
    connect(m_transfers.get(), &TransferCoordinator::operationFailed, this, &ChatCoordinator::operationFailed);
}
...
void ChatCoordinator::handleGroupSnapshot(const Network::GroupSnapshot &snapshot)
{
    ...
    {
        if (current.ownerId != snapshot.ownerId || current.revision >= snapshot.revision)
        {
            return;
        }
    }
    ...
    {
        group.members.append({member.peerId, member.displayName, member.owner ? Domain::GroupRole::Owner : Domain::GroupRole::Member});
    }
    if (!m_conversations->upsertGroup(group))
    {
        return;
...
void ChatCoordinator::dispatchPendingToPeer(const QString &peerId)
{
    ...
    const QList<Domain::MessageDelivery> deliveries = m_conversations->pendingDeliveriesForPeer(peerId);
...
Network::GroupSnapshot ChatCoordinator::networkSnapshot(const Domain::Group &group) const
{
    ...
    snapshot.sender = Network::PeerEndpoint{m_identity.deviceId, m_identity.displayName, {}, 0};
    for (const Domain::GroupMember &member : group.members)
    {
        snapshot.members.append({member.peerId, member.displayName, member.role == Domain::GroupRole::Owner});
    }
    return snapshot;
}
...
bool ChatCoordinator::resolveOnlineDirectPeer(const QString &conversationId, Domain::Peer *peerRecord, bool fileOperation)
{
    Domain::Conversation conversation;
    Domain::Peer peer;
    if (m_running && m_conversations->conversation(conversationId, &conversation) && conversation.kind == Domain::ConversationKind::Direct &&
