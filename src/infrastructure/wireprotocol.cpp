...
QByteArray Network::WireProtocol::textFrame(
    const LocalIdentity &identity, quint16 senderPort, const PeerEndpoint &recipient, const QString &messageId, const QString &text, const QDateTime &timestamp)
{
    QJsonObject object = envelope(identity, senderPort, recipient, QStringLiteral("message"), timestamp);
...
{
    QJsonObject object = envelope(identity, senderPort, recipient, QStringLiteral("file"), timestamp);
