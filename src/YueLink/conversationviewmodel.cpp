    ...
    const Domain::Message &message) const
{
    ...
    view.fileUrl = message.localAttachment.filePath.isEmpty()
                       ? QUrl{}
                       : QUrl::fromLocalFile(message.localAttachment.filePath);
    view.fromMe = message.metadata.senderId == m_coordinator->localIdentity().deviceId;
    m_coordinator->deliveryCounts(message.metadata.messageId,
                                  &view.deliveredCount,
...
QString ConversationViewModel::senderName(const Domain::Message &message) const
{
    ...
    for (const Domain::GroupMember &member :
         m_coordinator->groupMembers(message.metadata.conversationId))
    {
